...
{self.additional_info}"""
# <b>Последнее посещение:</b> {self.last_int_visit if hasattr(self, 'last_int_visit') else self.last_visit}<br>
class PDFExtractor:
    @staticmethod
...
class NotificationApp:
    ...
    def display_clean(self, data: NotificationData):
        ...
        fields = [("ФИО", f"{data.full_name}, {data.age}"), 
                  ("Дата поступления ЭИ", f"{data.notification_number} от {data.notification_date}"),
                  ("Место работы/учебы", data.organization), ("Последнее посещение", data.last_visit),
                  ("Дата заболевания", data.disease_date), ("Дата обращения", data.date_request),
                  ("Госпитализация", data._get_hosp_text()), ("Этиологическая расшифровка", "в работе"),
